...
/**
 * Cette classe va gérer les collisions avec les entités du jeu (les champignons
 * par exemple).
 * Elle va aussi gérer l'intéraction entre les entités et le joueur (mario).
 * Elle implémente la classe Thread.
 * Il faudra créer un thread par entité, donc un objet de cette classe par
 * entité.
 */
public class Collision_entite extends Thread {
    ...
    // Les positions des côtés du rectangle representant la hitbox de l'entité dans
    // la matrice de jeu.
    ...
    /**
     * Methode qui calcule les position en X et Y des 4 points de la hitbox de
     * l'entité.
    ...
    public void calculate_position_indexes() {
        ...
        this.posRightenX = this.entity.getPosition().x + this.entity.getSolidArea().x
                + this.entity.getSolidArea().width;
        this.posTopenY = this.entity.getPosition().y;

        this.posBottomenY = this.entity.getPosition().y + this.entity.getSolidArea().y
                + this.entity.getSolidArea().height * 2;
    ...
    public void run() {
        ...
        // 2 points, pour gérer les collisions, permet de savoir sur quel type de tuile
        // on fait face.
        int point1, point2;
        ...
        while (ok_thread) {
            try {
                ...
                int mario_border_right = this.mario.getPosition().x + this.mario.getSolidArea().x
                        + this.mario.getSolidArea().width;
                int mario_border_up = this.mario.getPositionY();
                int mario_border_down = this.mario.getPosition().y + this.mario.getSolidArea().y
                        + this.mario.getSolidArea().height * 2;

                // On regarde si la hitbox de mario et la hitbox de l'entité se chevauchent.
                // On vérifie si le point gauche de la hitbox de l'entité est dans la hitbox de
                // mario
                // ou si le point droit de la hitbox de l'entité est dans la hitbox de mario.
                if ((this.posLeftenX >= mario_border_left && this.posLeftenX <= mario_border_right)
                        || (this.posRightenX >= mario_border_left && this.posRightenX <= mario_border_right)) {
                    // Puis on vérifie si le point haut de la hitbox de l'entité est dans la hitbox
                    // de mario
                    // ou si le point bas de la hitbox de l'entité est dans la hitbox de mario.
                    if ((this.posTopenY >= mario_border_up && this.posTopenY <= mario_border_down)
                            || (this.posBottomenY >= mario_border_up && this.posBottomenY <= mario_border_down)) {
                        ...
                        if (this.entity instanceof Champignon) {
                            ...
                            ScoreManager.incrementCurrentScore("mushroom");
                            // On incrémente le nombre de champignon.
                            ScoreManager.incrementCurrentMushrooms();
                ...
                // On regarde les deux types de tuiles qui sont en dessous l'entité (en bas à
                ...
                posBottomenY = this.entity.getPosition().y + this.entity.getSolidArea().y
                        + this.entity.getSolidArea().height;
                ...
                switch (this.entity.getDirection()) {
                    ...
                    // Comment l'entité ne saute pas, on regarde juste ses collisions à gauche et à
                    // droite
                    case "left":
                        // On recalcule la colonne de la hitbox gauche de l'entité avec un intervale de
                        // collision (d'anticipation on va dire).
